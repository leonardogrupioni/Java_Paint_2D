package circulo;
import ponto.*;

/**
 * Escreva uma descrição da classe Circulo aqui.
 * 
 * @author (seu nome) 
 * @version (um número da versão ou uma data)
 */
public class Circulo
{
    
    // Atributos da reta
    public Ponto p1, p2;

    /**
     * Constroi uma reta com valores (int) de x1, y1 e x2, y2
     *
     * @param x1 coordenada x de p1
     * @param y1 coordenada y de p1
     * @param x2 coordenada x de p2
     * @param y2 coordenada y de p2
     */
    public Circulo(int x1, int y1, int x2, int y2) {
        setP1(new Ponto(x1, y1));
        setP2(new Ponto(x2, y2));
    }
    
    /**
     * Altera valor de p1 de acordo com o parametro
     *
     * @param p valor de p1 (externo)
     */
    public void setP1(Ponto p){
        this.p1 = p;
    }
    
    /**
     * Altera valor de p2 de acordo com o parametro
     *
     * @param p valor de p2 (externo)
     */
    public void setP2(Ponto p){
        this.p2 = p;
    }
    
    /**
     * Retorna valor de p1
     *
     * @return valor de p1
     */
    public Ponto getP1(){
        return this.p1;
    }
    
    /**
     * Retorna p2
     *
     * @return valor de p2
     */
    public Ponto getP2(){
        return this.p2;
    }
    
    public double calcularRaio(double x1, double x2, double y1, double y2){
        double raio = Math.sqrt( Math.pow(x2 - x1,2) + Math.pow(y2 - y1,2) );
        return raio;
    }
    
    public double calcularX(double x1, double raio, double a){
        double rA = Math.toRadians(a); //angulo * (pi/180)
        double x = (int) (x1 + raio * Math.cos(rA)); //coseno do angulo * raio + x central 
        return x;
    }
    
    public double calcularY(double y1, double raio, double a){
        double rA = Math.toRadians(a); //angulo * (pi/180)
        double y = (int) (y1 + raio * Math.sin(rA)); //seno do angulo * raio + y central
        return y;
    }   
    
}
